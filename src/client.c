    ...
    while (1)
    {
        ...
        case 1:
            ...
            else if (result == INCORRECT_ACC)
            {
                ...
                printf("\n Mật khẩu hoặc tài khoản không đúng! Vui lòng nhập lại \n");
            }
            else
            {
                report_err(ERR_SIGNED_IN_ACC);
                printf("\n Tài khoản này đã được đăng nhập trước đó! \n");
            }
            break;
        case 2:
...
void private_chat(int client_socket)
{
    ...
    while (1)
    {
        printf("Message(just press 'enter' to exit private chat): \n");
...
void chat_all(int client_socket)
{
    ...
    while (1)
    {
        printf("Message(just press 'enter' to exit group chat): \n");
        fgets(msg, MSG_SIZE, stdin);
        msg[strlen(msg) - 1] = '\0';
        if (strlen(msg) == 0)
        {
